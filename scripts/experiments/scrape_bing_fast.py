...
парсит выдачу через regex, скачивает параллельно
дедуп по phash, фильтр по размеру
"""
...
fieldnames = ["id", "filename", "original_name", "query_tag", "width", "height", "phash"]
...
for i, query in enumerate(QUERIES, 1):
    ...
    with ThreadPoolExecutor(max_workers=MAX_WORKERS) as executor:
        futures = []
        for url in links:
            futures.append(executor.submit(download_image, url, query, seen_hashes))
        for future in futures:
            ...
            if res:
                ...
                with open(METADATA_FILE, "a", newline="", encoding="utf-8") as f:
                    writer = csv.DictWriter(f, fieldnames=fieldnames)
                    writer.writerow(res)
    print(f"{i}/{len(QUERIES)} {query!r} saved={success_count} total={total_saved}")

print(f"готово total={total_saved}")
