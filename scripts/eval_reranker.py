...
# ============ ШАГ 1: encode все запросы и получить top-10 через RRF ============
print("\n=== Шаг 1: encode + RRF (bge-m3) ===")
from sentence_transformers import SentenceTransformer
encoder = SentenceTransformer("BAAI/bge-m3", device="cpu")

weights = [0.6, 0.4]
# Для каждого (item, query, lang) сохраняем top-10 и target_idx
tasks = []  # [(lang, query_text, target_idx, top10_indices)]
...
print("bge-m3 выгружен")

# ============ ШАГ 2: rerank top-10 ============
print("\n=== Шаг 2: rerank (CrossEncoder) ===")
...
for lang, query, target_idx, top10 in tqdm(tasks, desc="Reranking"):
    ...
    r = results[lang]
    if target_idx in reranked[:1]: r["h1"] += 1
    if target_idx in reranked[:5]: r["h5"] += 1
    if target_idx in reranked[:10]: r["h10"] += 1
...
for lang in ["en", "ru"]:
    r = results[lang]
    t = r["total"]
    print(f"\n=== {lang.upper()} + reranker ({t} запросов) ===")
